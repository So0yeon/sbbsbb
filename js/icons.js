...
(function (g) {
  ...
  /* ── 자료에 남은 이모지 걷어내기 ─────────────────────────────
     ...
     화살표(→ ← ↺)와 홑화살괄호(‹ ›)는 걷어내지 않는다.
     그것은 그림이 아니라 활자다 — 단추의 '알겠소 →' 같은 자리에 쓰이고,
     여기서 지워 버리면 문장이 어색해진다. */
  var EMOJI_RE = /[\u{1F000}-\u{1FAFF}\u{2300}-\u{27BF}\u{2B00}-\u{2BFF}\u{FE0F}\u{200D}\u{E0020}-\u{E007F}]/gu;
  function strip(s){
    if (s == null) return '';
